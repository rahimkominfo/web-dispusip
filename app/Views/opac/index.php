...
<nav aria-label="Breadcrumb" class="mb-6 font-label-md text-label-md text-on-surface-variant flex items-center gap-2 flex-wrap">
    ...
    <i class="fa-solid fa-chevron-right text-[12px] opacity-60"></i>
    <span aria-current="page" class="text-on-surface font-semibold">OPAC Perpustakaan</span>
</nav>

<!-- Main Container Card -->
<div class="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden shadow-sm mb-12" id="opac-wrapper">
        ...
        <div class="space-y-2">
            <div class="flex items-center gap-3 flex-wrap">
                <span class="px-3 py-1 text-xs font-semibold uppercase tracking-wider rounded-full bg-primary/10 text-primary border border-primary/20 flex items-center gap-1.5">
                    <i class="fa-solid fa-book-bookmark"></i> INLISLite OPAC
                </span>
            ...
            <p class="text-on-surface-variant font-body-md text-sm md:text-base max-w-3xl">
                Cari dan temukan koleksi buku, jurnal, referensi, dan bahan pustaka yang tersedia di Perpustakaan Daerah Kabupaten Sinjai secara langsung.
            </p>
        </div>

        <!-- Action Buttons Bar -->
        <div class="flex items-center gap-2 shrink-0 flex-wrap">
            <button id="btn-refresh-iframe" type="button" title="Muat Ulang Katalog"
    ...
    <div class="relative w-full bg-slate-900 overflow-hidden" id="iframe-box" style="height: 2500px;">
        <!-- Loading Spinner Overlay -->
        <div id="opac-loading" class="absolute inset-0 z-20 bg-surface/90 backdrop-blur-sm flex flex-col items-center justify-center gap-4 transition-opacity duration-500">
            ...
            <div class="text-center space-y-1">
                <p class="font-medium text-on-surface text-base">Memuat Katalog OPAC Sinjai...</p>
                <p class="text-xs text-on-surface-variant">Menghubungkan ke server katalog perpustakaan</p>
            </div>
        </div>

        <!-- Embedded OPAC Iframe -->
        <iframe 
            id="opac-iframe"
            src="<?= esc($opacUrl) ?>" 
            title="OPAC Dinas Perpustakaan dan Kearsipan Sinjai"
    ...
    <div class="p-4 md:p-6 bg-surface-container border-t border-outline-variant flex items-center gap-3 text-xs md:text-sm text-on-surface-variant">
        ...
        <span>Layanan Online Public Access Catalog (OPAC) Perpustakaan Daerah Kabupaten Sinjai.</span>
    </div>
</div>

<script>
    function onOpacLoaded() {
        const loading = document.getElementById('opac-loading');
        const iframe = document.getElementById('opac-iframe');
        if (loading) {
        ...
        if (loading) {
            loading.innerHTML = `
                <div class="text-center p-6 max-w-md bg-surface border border-outline-variant rounded-xl shadow-lg space-y-3">
                    ...
                    <h3 class="font-bold text-on-surface text-lg">Katalog Tidak Dapat Dimuat di Frame</h3>
                    <p class="text-xs text-on-surface-variant leading-relaxed">
                        Server OPAC tidak mengizinkan tampilan embed atau koneksi dibatasi. Silakan klik tombol di bawah untuk membuka langsung di tab baru.
                    </p>
                    <a href="<?= esc($opacUrl) ?>" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg bg-primary text-on-primary font-semibold text-sm transition-colors hover:bg-primary/90">
    ...
    document.getElementById('btn-fullscreen-iframe')?.addEventListener('click', function() {
        const wrapper = document.getElementById('opac-wrapper');

...
<div class="mb-8">
    ...
    <h2 class="font-headline-lg text-headline-lg text-primary">Edit Kategori</h2>
</div>
...
<div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-6 max-w-2xl">
    <form action="<?= base_url('admin/categories/update/' . $category['kategori_id']) ?>" method="POST" class="flex flex-col gap-6">
        <?= csrf_field() ?>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex flex-col gap-1">
                <label class="font-label-md text-label-md text-on-surface" for="nama">Nama Kategori *</label>
                <input required type="text" name="nama" id="nama" value="<?= old('nama', esc($category['nama'])) ?>" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow"/>
            </div>

            <div class="flex flex-col gap-1">
                <label class="font-label-md text-label-md text-on-surface" for="slug">Slug URL *</label>
                <input required type="text" name="slug" id="slug" value="<?= old('slug', esc($category['slug'])) ?>" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow" placeholder="seo-friendly-slug-url"/>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-1">
            ...
            <select name="kategori_induk_id" id="kategori_induk_id" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow">
                ...
                <?php foreach ($parentCategories as $parent): ?>
                    <option value="<?= esc($parent['kategori_id']) ?>" <?= old('kategori_induk_id', esc($category['kategori_induk_id'])) == $parent['kategori_id'] ? 'selected' : '' ?>><?= esc($parent['nama']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-outline-variant">
            <button type="submit" class="bg-primary text-on-primary hover:bg-surface-tint rounded px-6 py-2 font-label-md text-label-md font-semibold transition-colors flex items-center gap-2">
                <span class="material-symbols-outlined text-sm">save</span> Perbarui Kategori
            </button>

...
<?= $this->section('title') ?>Edit Running Text<?= $this->endSection() ?>
...
<div class="mb-8">
    ...
    <h2 class="font-headline-lg text-headline-lg text-primary">Edit Running Text</h2>
</div>
...
<div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-6 max-w-2xl">
    <form action="<?= base_url('admin/running-text/update/' . $runningText['id']) ?>" method="POST" class="flex flex-col gap-6">
        <?= csrf_field() ?>
        
        <div class="flex flex-col gap-1">
            <label class="font-label-md text-label-md text-on-surface" for="teks">Isi Teks Pengumuman *</label>
            <textarea required name="teks" id="teks" rows="4" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow" placeholder="Masukkan teks pengumuman yang akan berjalan..."><?= old('teks', esc($runningText['teks'])) ?></textarea>
        </div>
        ...
        <div class="flex flex-col gap-1">
            ...
            <select required name="is_active" id="is_active" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow">
                <option value="1" <?= old('is_active', esc($runningText['is_active'])) == '1' ? 'selected' : '' ?>>Aktif</option>
                <option value="0" <?= old('is_active', esc($runningText['is_active'])) == '0' ? 'selected' : '' ?>>Tidak Aktif</option>
            </select>
        </div>
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-outline-variant">
            <button type="submit" class="bg-primary text-on-primary hover:bg-surface-tint rounded px-6 py-2 font-label-md text-label-md font-semibold transition-colors flex items-center gap-2">
                <span class="material-symbols-outlined text-sm">save</span> Perbarui Running Text
            </button>

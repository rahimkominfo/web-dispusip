...
<?= $this->section('title') ?>Tambah Menu Navigasi<?= $this->endSection() ?>
...
<div class="mb-8">
    ...
    <h2 class="font-headline-lg text-headline-lg text-primary">Tambah Menu Baru</h2>
</div>
...
<div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-6 max-w-2xl">
    <form action="<?= base_url('admin/menus/store') ?>" method="POST" class="flex flex-col gap-6">
        <?= csrf_field() ?>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex flex-col gap-1">
                <label class="font-label-md text-label-md text-on-surface" for="title">Label Menu *</label>
                <input required type="text" name="title" id="title" value="<?= old('title') ?>" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow" placeholder="Contoh: Berita, Profil, dsb."/>
            </div>
            
            <div class="flex flex-col gap-1">
                <label class="font-label-md text-label-md text-on-surface" for="url">URL Menu *</label>
                <input required type="text" name="url" id="url" value="<?= old('url') ?>" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow" placeholder="Contoh: berita, profil/visi-misi, https://..."/>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="flex flex-col gap-1">
                ...
                <select name="parent_id" id="parent_id" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow">
                    ...
                    <?php foreach ($parentMenus as $parent): ?>
                        <option value="<?= esc($parent['id']) ?>" <?= old('parent_id') == $parent['id'] ? 'selected' : '' ?>><?= esc($parent['title']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="flex flex-col gap-1">
                ...
                <input required type="number" min="0" name="sort_order" id="sort_order" value="<?= old('sort_order', '0') ?>" class="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 text-on-surface focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 focus:outline-none transition-shadow" placeholder="Contoh: 1, 2, 3"/>
            </div>
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-outline-variant">
            <button type="submit" class="bg-primary text-on-primary hover:bg-surface-tint rounded px-6 py-2 font-label-md text-label-md font-semibold transition-colors flex items-center gap-2">
                <span class="material-symbols-outlined text-sm">save</span> Simpan Menu
            </button>
